...
type RenterApplicationActionsProps = {
  id: string;
  status: string;
};
...
export function RenterApplicationActions({ id, status }: RenterApplicationActionsProps) {
  ...
  async function runAction(action: "approve" | "request_docs" | "decline") {
    if (!isPersistedId(id)) {
      setError("Sample application only. Submit a real renter application to review live records.");
      return;
    }
    ...
    try {
      const res = await fetch(`/api/admin/renter-applications/${id}`, {
        method: "PATCH",
